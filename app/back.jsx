...
const EXERCISE_PREVIEW_IMAGES = {
  ...
  "Barbell Back Squat": "https://images.unsplash.com/photo-1574680096145-d05b474e2155?q=80&w=600&auto=format&fit=crop",
  "Leg Press": "https://images.unsplash.com/photo-1434682881908-b43d0467b798?q=80&w=600&auto=format&fit=crop",
  "Hack Squat": "https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=600&auto=format&fit=crop",
  "Goblet Squat": "https://images.unsplash.com/photo-1571019614242-c5c5dee9f50b?q=80&w=600&auto=format&fit=crop",
  "Dumbbell Bulgarian Split Squat": "https://images.unsplash.com/photo-1541534741688-6078c6bfb5c5?q=80&w=600&auto=format&fit=crop",
  "Leg Extensions": "https://images.unsplash.com/photo-1517838277536-f5f99be501cd?q=80&w=600&auto=format&fit=crop",
  "Dumbbell Walking Lunges": "https://images.unsplash.com/photo-1605296867304-46d5465a25f1?q=80&w=600&auto=format&fit=crop",
  "Smith Machine Squat": "https://images.unsplash.com/photo-1574680096145-d05b474e2155?q=80&w=600&auto=format&fit=crop",
  "Smith Machine Calf Raise": "https://images.unsplash.com/photo-1574680096145-d05b474e2155?q=80&w=600&auto=format&fit=crop",
  "Donkey Calf Raise": "https://images.unsplash.com/photo-1571019614242-c5c5dee9f50b?q=80&w=600&auto=format&fit=crop",
  "Dumbbell Single-Leg Calf Raise": "https://images.unsplash.com/photo-1581009146145-b5ef050c2e1e?q=80&w=600&auto=format&fit=crop",
  "Bodyweight Calf Raise": "https://images.unsplash.com/photo-1598971639058-aba3c7f09a7d?q=80&w=600&auto=format&fit=crop",
  "Jump Rope": "https://images.unsplash.com/photo-1517838277536-f5f99be501cd?q=80&w=600&auto=format&fit=crop"
};
...
const BACK_MAIN_CATEGORIES = [
  ...
  { 
    key: "lowerBack", label: "Lower Back", bg: "lowerback", 
    data: [
  ...
  { 
    key: "clavicularHead", label: "Clavicular Head", bg: "upperback", 
    data: [
  ...
  { 
    key: "sternalHead", label: "Sternal Head", bg: "midback", 
    data: [
  ...
  {
    key: "triceps", label: "Triceps", bg: "biceps", 
    data: [
      { id: "1", name: "Triceps Pushdown (Cable)" }, 
      { id: "2", name: "Overhead Triceps Extension" }, 
      { id: "3", name: "Skull Crushers (EZ-Bar)" },
      { id: "4", name: "Close-Grip Bench Press" },
      { id: "5", name: "Triceps Dips" },
      { id: "6", name: "Dumbbell Kickbacks" },
      { id: "7", name: "Single-Arm Cable Pushdown" }
    ] 
  }
];

const BACK_LEGS_CATEGORIES = [
  {
    key: "quadriceps", label: "Quadriceps Focus", bg: "lowerback",
    data: [
      { id: "1", name: "Barbell Back Squat" },
      { id: "2", name: "Leg Press" },
      { id: "3", name: "Hack Squat" },
      { id: "4", name: "Goblet Squat" },
      { id: "5", name: "Dumbbell Bulgarian Split Squat" },
      { id: "6", name: "Leg Extensions" },
      { id: "7", name: "Dumbbell Walking Lunges" },
      { id: "8", name: "Smith Machine Squat" }
    ]
  },
  { 
    key: "medialDelts", label: "Medial Delts", bg: "shoulders", 
    data: [
  ...
  {
    key: "hamstrings", label: "Hamstrings", bg: "lowerback", 
    data: [
      { id: "1", name: "Romanian Deadlift" },
  ...
  { 
    key: "glutes", label: "Glutes", bg: "midback", 
    data: [
  ...
  {
    key: "calves", label: "Calves", bg: "shoulders",
    data: [
      { id: "1", name: "Standing Calf Raise" },
      { id: "2", name: "Seated Calf Raise" },
      { id: "3", name: "Leg Press Calf Raise" },
      { id: "4", name: "Smith Machine Calf Raise" },
      { id: "5", name: "Donkey Calf Raise" },
      { id: "6", name: "Dumbbell Single-Leg Calf Raise" },
      { id: "7", name: "Bodyweight Calf Raise" },
      { id: "8", name: "Jump Rope" }
    ]
  },
  { 
    key: "adductors", label: "Adductors", bg: "lowerback", 
    data: [
  ...
  { 
    key: "forearms", label: "Forearms", bg: "biceps", 
    data: [
  ...
  { 
    key: "core", label: "Core", bg: "midback", 
    data: [
      { id: "1", name: "Plank" },
...
export default function BackDominantSplit() {
    ...
    const fetchCloudUserProgressData = async () => {
      try {
        ...
        if (userSnap.exists()) {
          ...
          if (userData.backSplitSelections) {
  ...
  const handleSelect = async (categoryKey, exerciseName) => {
    ...
    try {
      ...
      if (user) {
        const userRef = doc(db, "users", user.uid);
        await setDoc(userRef, { backSplitSelections: updatedSelections }, { merge: true });
      }
    } catch (error) {
  ...
  const handlePlayWorkoutSession = (exerciseName, categoryKey) => {
    ...
    const trackingKey = `backDom_${activeDay}_${categoryKey}_${exerciseName}`;
  ...
  const finishActiveSetExecution = async () => {
    ...
    const trackingKey = `backDom_${activeDay}_${activeCategoryKey}_${activeExercise}`;
      ...
      try {
        ...
        if (user) {
          ...
          await updateDoc(userRef, { 
            ...
            backSplitWorkoutProgress: updatedWorkoutProgress 
          });
        }
      } catch (err) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.mainTitle}>Back Dominant Split</Text>
      ...
      <View style={styles.navRow}>
        <Pressable 
          ...
          onPress={() => { setActiveDay("WARM_UP"); setExpandedCategory(null); }}
        >
          <Text style={[styles.navPillText, activeDay === "WARM_UP" && styles.activeNavPillText]}>Warm Up</Text>
        </Pressable>
        <Pressable 
          style={[styles.navPill, activeDay === "MAIN" && styles.activeNavPill]} 
          onPress={() => { setActiveDay("MAIN"); setExpandedCategory(null); }}
        >
          <Text style={[styles.navPillText, activeDay === "MAIN" && styles.activeNavPillText]}>Back & Chest</Text>
        </Pressable>
        <Pressable 
          style={[styles.navPill, activeDay === "LEGS" && styles.activeNavPill]} 
          onPress={() => { setActiveDay("LEGS"); setExpandedCategory(null); }}
        >
          <Text 
            style={[styles.navPillText, activeDay === "LEGS" && styles.activeNavPillText]}
            numberOfLines={1}
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        {currentCategories.map((cat) => {
          ...
          const bgUri = selectedExercise 
            ? (EXERCISE_PREVIEW_IMAGES[selectedExercise] || CATEGORY_BACKGROUNDS[cat.bg]) 
            : CATEGORY_BACKGROUNDS[cat.bg];
          
          let isCompleted = false;
          if (selectedExercise) {
            const trackingKey = `backDom_${activeDay}_${cat.key}_${selectedExercise}`;
            ...
            <View key={cat.key} style={styles.cardWrapper}>
              <View style={[styles.cardTouchTarget, selectedExercise && { height: 130 }]}>
                {selectedExercise ? (
                  <ImageBackground source={{ uri: bgUri }} style={styles.imageBackground} imageStyle={{ borderRadius: 24 }}>
                    ...
                    <View style={styles.cardHeaderContainer}>
                      ...
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <Pressable 
                          style={[styles.startWorkoutBtn, isCompleted && { backgroundColor: '#1e293b' }]} 
                          onPress={() => !isCompleted && handlePlayWorkoutSession(selectedExercise, cat.key)}
                        >
                          <Text style={[styles.startWorkoutBtnText, isCompleted && { color: '#94a3b8' }]}>
                            {isCompleted ? "✓ DONE" : "▶ START"}
                          </Text>
                        </Pressable>
                        <Pressable 
                          onPress={() => setExpandedCategory(isExpanded ? null : cat.key)} 
                          style={styles.caretContainerCircle}
                        >
                  ...
                  <Pressable 
                    style={styles.blankExerciseCardBody}
                    onPress={() => setExpandedCategory(isExpanded ? null : cat.key)}
                  >
                    <View style={{ flex: 1 }}>
                ...
                <View style={styles.dropdownOptionsContainer}>
                  {cat.data.map((exercise) => {
                    ...
                    return (
                      <Pressable
                        key={exercise.id}
                        onPress={() => handleSelect(cat.key, exercise.name)}
                        style={[
                          styles.exerciseListItemBlock,
                          isSelected && { borderColor: '#f97316', backgroundColor: '#1c2538' }
                        ]}
                      >
                        <Text style={[styles.exerciseNameText, isSelected && { color: '#f97316' }]}>
                          {exercise.name}
                        </Text>
                        {isSelected && (
        ...
        <View style={styles.kineticInsightCard}>
          ...
          <Text style={styles.insightQuoteContent}>
            "Prioritize thick rowing movements today to maximize spinal erector engagement. The back-dominant split is designed for total posterior chain hypertrophy."
          </Text>
        </View>
      </ScrollView>
        ...
        <ScrollView contentContainerStyle={styles.playerWrapperContainer} bounces={false} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.timerControlCenterDashboard}>
            ...
            <Text style={[styles.paceWarningIndicatorText, !isCompletionGateUnlocked && { color: '#EF4444' }]}>
              {isCompletionGateUnlocked ? "Safe Execution Gate Complete — Unlocked" : `Complete movement path (${Math.max(0, requiredSafetySeconds - secondsSpent)}s left)`}
            </Text>
          </View>
          ...
          <View style={styles.footerActionDashboardZone}>
            <Pressable 
              style={[styles.giantSuccessVerificationButton, !isCompletionGateUnlocked && { backgroundColor: '#252233', opacity: 0.6 }]} 
              onPress={finishActiveSetExecution} 
              disabled={!isCompletionGateUnlocked}
            >
              <Text style={styles.successActionBtnContentText}>
                {isLastSet ? "FINISH EXERCISE 🏆" : "COMPLETE SET & REST ⚡"}
              </Text>
            </Pressable>
          </View>
        </ScrollView>
